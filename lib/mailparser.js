...
// Optionally use ICU to auto detect content charset
var charsetDetector;
try{
    charsetDetector = require("node-icu-charset-detector");
}catch(e){
    charsetDetector = null;
}
...
MailParser.prototype.write = function(chunk, encoding){
    // Cache data for content charset detection
...
MailParser.prototype._processHeaderLine = function(pos){
    ...
    this._markFieldContainMimeWord(key, value);
    ...
    switch(key){
        ...
        case "mime-version":
            ...
            break;
        case "date":
            this._currentNode._dateString = value;
        ...
        case "content-transfer-encoding":
            ...
            break;
        case "subject":
            this._currentNode.subject = this._containMimeWord(value) ? this._encodeString(value) : value;
            break;
        ...
        case "content-id":
            ...
            break;
        // Parse Received: header for retrieve received date
        case "received":
            this._currentNode.receivedDateString = value.substring(value.lastIndexOf(";"));
            break;
    }
...
MailParser.prototype._processMimeTree = function(){
    ...
    this._smartConvertFieldsValue();
    if(this.mimeTree._debugNotMimeEncoded){
        returnValue._debugNotMimeEncoded = true;
        returnValue._debugMetaCharset = this.mimeTree._debugMetaCharset;
        returnValue._debugDetectedCharset = this.mimeTree._debugDetectedCharset;
    }
    ...
    var receivedDateString = this.mimeTree.receivedDateString || this.mimeTree._dateString || "";
    var receivedTimestamp = (new Date(receivedDateString)).getTime();
    if(!isNaN(receivedTimestamp)){
        returnValue.received = receivedTimestamp;
    }
...
MailParser.prototype._containMimeWord = function(value){
    value = (value || "").toString().trim();
    return (new RegExp("\=\?([\w_\-]+)\?([QqBb])\?([^\?]+)\?\=", "ig")).test(value);
};

MailParser.prototype._markFieldContainMimeWord = function(field, value){
    this._currentNode._fieldContainMimeWordMap || (this._currentNode._fieldContainMimeWordMap = {});
    if("to,from,reply-to,cc,bcc,subject".split(",").indexOf(field) !== -1){
        field === "reply-to" && (field = "replyTo");
        this._currentNode._fieldContainMimeWordMap[field] = this._containMimeWord(value);
    }
};

MailParser.prototype._smartConvertFieldsValue = function(){
    var self = this;
    "to,from,replyTo,cc,bcc,subject".split(",").forEach(function(field){
        if(self.mimeTree[field] && self.mimeTree._fieldContainMimeWordMap[field] === false){
            if(Array.isArray(self.mimeTree[field])){ // to,from,replyTo,cc,bcc
                self.mimeTree[field].forEach(function(item){
                    item.name = self._smartConvertStringToUTF8(item.name);
                });
            }else{ // subject
                self.mimeTree._debugNotMimeEncoded = true;
                self.mimeTree[field] = self._smartConvertStringToUTF8(self.mimeTree[field]);
            }
        }
    });

    this._streamData = null;
    delete this._streamData;
};

MailParser.prototype._smartConvertStringToUTF8 = function(value){
    this.mimeTree._debugMetaCharset = this._currentNode.meta.charset;

    return this._convertString(value, this._detectStreamCharset() || this._currentNode.meta.charset || this.options.defaultCharset || "iso-8859-1").toString("utf-8");
};

MailParser.prototype._detectStreamCharset = function(){
    if(!charsetDetector || !Buffer.isBuffer(this._streamData)){
        return "";
    }

    var charset = this.mimeTree._debugDetectedCharset = charsetDetector.detectCharset(this._streamData).toString();
